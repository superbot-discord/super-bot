...
async def user(ctx, user: discord.Member = None, channel: discord.TextChannel = None):
  ...
  f1ts = str(datetime.datetime.now() - user.created_at)
  if f1ts.count(" days, ") == 0:
    f1va = re.sub('(\d{1,2}):(\d{2}):(\d{2})', r'\1 hours \2 minutes \3 seconds', f1ts) + f"\n≈ {f1ts.split(":")[0]} hours"
  else:
    days = int(re.sub('([\d]+) days, [\s\S]*', r'\1', f1ts))
    f1va = re.sub('([\d]+) days, (\d{1,2}):(\d{2}):(\d{2})', r'\1 days \2 hrs \3 mins \4 secs', f1ts)[:-7] + f"\n≈ {(f1ts.split(" days, ")[0])//365} years {(f1ts.split(" days, ")[0]) % 365} days"
  f2v=user.joined_at.strftime("%d %b, %Y (%a) %H:%M:%S")
  f2ts = str(datetime.datetime.now() - user.joined_at)
  if f2ts.count(" days, ") == 0:
    f2va = re.sub('(\d{1,2}):(\d{2}):(\d{2})', r'\1 hours \2 minutes \3 seconds', f2ts) + f"\n≈ {f2ts.split(":")[0]} hours"
  else:
    f2va = re.sub('([\d]+) days, (\d{1,2}):(\d{2}):(\d{2})', r'\1 days \2 hrs \3 mins \4 secs', f2ts)[:-7] + f"\n≈ {(f2ts.split(" days, ")[0])//365} years {(f2ts.split(" days, ")[0]) % 365} days"
  allroles=user.roles
  f3v=""
