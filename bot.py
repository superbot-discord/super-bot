...
async def user(ctx, user: discord.Member = None, channel: discord.TextChannel = None):
  ...
  if f3vcraw.type.playing:
    try:
      f3vc = f"Playing {f3vcraw.name} since {f3vcraw.start.strftime("%d %b, %Y (%a) %H:%M:%S")}\n{f3vcraw.details}"
    except:
    ...
    f3vc = f"Listening to {f3vcraw.artist} - {f3vcraw.album}: {f3vcraw.title}\nStarted: {f3vcraw.created_at.strftime("%d %b, %Y (%a) %H:%M:%S")}\n{f3vcraw.details}"
  elif f3vcraw.type.watching:
    try:
      f3vc = f"Watching [{f3vcraw.name}]({f3vcraw.url}) since {f3vcraw.start.strftime("%d %b, %Y (%a) %H:%M:%S")}\n{f3vcraw.details}"
    except:
